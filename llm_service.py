...
class LLMService:
    def __init__(self, url, model="local-model"):
        # For OpenAI-compatible API, url should be the base URL (e.g., http://192.168.12.216:1234/v1)
    ...
    def _talk_to_llm(self, messages, temperature=0.7):
        try:
            ...
            response = requests.post(self.chat_url, json=payload, timeout=180)  # Increased timeout to 3 minutes
            
            response.raise_for_status()
            content = response.json()["choices"][0]["message"]["content"]
            
            # Clean up content - remove <think> tags and other reasoning artifacts
            # Some reasoning models wrap their thought process in tags
